...
void nfc_magic_scene_slix_save_name_text_input_done_callback(void* context) {
    NfcMagicApp* instance = context;

    view_dispatcher_send_custom_event(instance->view_dispatcher, NfcMagicCustomEventTextInputDone);
}

void nfc_magic_scene_slix_save_name_on_enter(void* context) {
    NfcMagicApp* instance = context;

    // Load the read ISO15693 card into source_dev so nfc_device_save() can serialise it. We only
    // hold iso15693_3 data (UID + system info + blocks), so the file is a plain ISO15693-3 .nfc.
    nfc_device_set_data(
        instance->source_dev,
        NfcProtocolIso15693_3,
        instance->slix_data->iso15693_3_data);
    ...
    text_input_set_result_callback(
        text_input,
        nfc_magic_scene_slix_save_name_text_input_done_callback,
        instance,
        instance->text_store,
...
bool nfc_magic_scene_slix_save_name_on_event(void* context, SceneManagerEvent event) {
    NfcMagicApp* instance = context;
    bool consumed = false;
...
void nfc_magic_scene_slix_save_name_on_exit(void* context) {
    NfcMagicApp* instance = context;
