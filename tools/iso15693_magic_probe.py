...
def inventory_render(inv, path=None):
    ...
    try:
        ...
        with open(path, "w") as f:
            ...
            f.write("A tag marked **\\*** is the exception: its snapshot is the earliest READ we have, taken\n")
            f.write("after a write had already happened, so it records a state the tag was PUT IN, not one it\n")
            f.write("came in. See that tag's note in the JSON before citing any field of it as silicon.\n")
