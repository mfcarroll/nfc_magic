...
def inventory_render(inv, path=None):
    ...
    for card in sorted(inv.get("tags", {})):
        ...
        if (e.get("note") or "").lstrip().startswith("\u26a0"):
            tag_cell = "\u26a0\ufe0f " + tag_cell
    ...
    try:
        ...
        with open(path, "w") as f:
            ...
            hazards = [(c, inv["tags"][c].get("note") or "") for c in sorted(inv.get("tags", {}))
                       if (inv["tags"][c].get("note") or "").lstrip().startswith("\u26a0")]
            if hazards:
                f.write("## \u26a0\ufe0f Read this before touching hardware\n\n")
                for c, note in hazards:
                    first = note.lstrip().split(" || ")[0].strip()
                    f.write("- **`%s`** \u2014 %s\n" % (c, first))
                f.write("\n")
